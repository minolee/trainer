...
class FormatConfig(BaseConfig):
    """Instance을 실제 학습 input string으로 가공할 때 사용할 prompt template와 데이터 형태를 정의하는 config class"""
    
    prompt: str | CallConfig
    """Instance을 실제 학습 input string으로 가공할 때 사용할 prompt template. 모델별로 정의할 것"""
    
    format: str | CallConfig
    """preference, rank, sft 등의 데이터 형태, Trainer에 따라 다르게 정의할 것"""

    def __call__(self, stage: str, data: list[Instance], tokenizer: PreTrainedTokenizer):
